...
@section('pageInfo')
    <div class="page-header-title">
        <h5 class="m-b-10">Редактирование категории "{{$category->name}}"</h5>
    </div>
    <ul class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{route('admin.home')}}"><i class="feather icon-home"></i></a></li>
        <li class="breadcrumb-item"><a href="{{route('admin.categories.index')}}">Список категорий</a></li>
        <li class="breadcrumb-item"><a href="#!">Редактирование категории "{{$category->name}}"</a></li>
    </ul>
@endsection
@section('content')
    <div class="col-sm-12">
        <div class="card">
            <div class="card-body">
                <div class="col">
                    <form role="form" action="{{route('admin.categories.update', ['id' => $category->id])}}" enctype="multipart/form-data" method="post">
                        @csrf
                        @method('PUT')
                        <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description" id="classic-editor">{{$category->description}}</textarea>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" name="status" type="checkbox" id="status" {{ $category->status==1?'checked':'' }}>
                                    <label class="form-check-label" for="status">Категория активна</label>
                                </div>
                            </div>
                            <div class="form-group col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" name="popular" type="checkbox" id="popular" {{ $category->popular==1?'checked':'' }}>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-4">
                                ...
                                <input type="text" value="{{$category->meta_title}}" name="meta_title" class="form-control" id="inputEmail4" placeholder="" required>
                            </div>
                            <div class="form-group col-md-4">
                                <label for="inputPassword4">Description</label>
                                <input type="text" value="{{$category->meta_descrip}}" class="form-control" name="meta_descrip"  id="inputPassword4" placeholder="" required>
                            </div>
                            <div class="form-group col-md-4">
                                <label for="inputPassword4">Keywords</label>
                                <input type="text" class="form-control" value="{{$category->meta_keywords}}" name="meta_keywords"  id="inputPassword4" placeholder="" required>
                            </div>
                        </div>
                        @if($category->image)
                        <div class="form-row py-5">
                            <img src="{{asset('assets/uploads/category/'.$category->image)}}" style="width: 150px" alt="">
                        </div>
                        @endif
                        <div class="form-group">
